...
IF /I "%SIGN%" == "True" (
  SET FILES="%~dp0_bin\Filters_x86%SUFFIX%\MpcImageSource.ax" "%~dp0_bin\Filters_x64%SUFFIX%\MpcImageSource64.ax"
  CALL "%~dp0\sign.cmd" !FILES! || (CALL :SubMsg "ERROR" "Problem signing !FILES!" & EXIT /B)
  CALL :SubMsg "INFO" "!FILES! signed successfully."
)

FOR /F "tokens=3,4 delims= " %%A IN (
  'FINDSTR /I /L /C:"define MPCIS_VERSION_MAJOR" "Include\Version.h"') DO (SET "VERMAJOR=%%A")
FOR /F "tokens=3,4 delims= " %%A IN (
  'FINDSTR /I /L /C:"define MPCIS_VERSION_MINOR" "Include\Version.h"') DO (SET "VERMINOR=%%A")
FOR /F "tokens=3,4 delims= " %%A IN (
  'FINDSTR /I /L /C:"define MPCIS_VERSION_BUILD" "Include\Version.h"') DO (SET "VERBUILD=%%A")
FOR /F "tokens=3,4 delims= " %%A IN (
  'FINDSTR /I /L /C:"define MPCIS_RELEASE" "Include\Version.h"') DO (SET "RELEASE=%%A")
FOR /F "tokens=3,4 delims= " %%A IN (
  'FINDSTR /I /L /C:"define MPCIS_REV_DATE" "revision.h"') DO (SET "REVDATE=%%A")
FOR /F "tokens=3,4 delims= " %%A IN (
  'FINDSTR /I /L /C:"define MPCIS_REV_HASH" "revision.h"') DO (SET "REVHASH=%%A")
FOR /F "tokens=3,4 delims= " %%A IN (
  'FINDSTR /I /L /C:"define MPCIS_REV_NUM" "revision.h"') DO (SET "REVNUM=%%A")

IF /I "%RELEASE%" == "1" (
  SET "PCKG_NAME=MPCImageSource-v%VERMAJOR%.%VERMINOR%.%VERBUILD%.%REVNUM%%SUFFIX%"
) ELSE (
  SET "PCKG_NAME=MPCImageSource-v%VERMAJOR%.%VERMINOR%.%VERBUILD%.%REVNUM%_git%REVDATE%-%REVHASH%%SUFFIX%"
)
...
.\_bin\Filters_x86%SUFFIX%\MpcImageSource.ax ^
.\_bin\Filters_x64%SUFFIX%\MpcImageSource64.ax ^
.\distrib\Install_MPCImageSource_32.cmd ^
.\distrib\Install_MPCImageSource_64.cmd ^
.\distrib\Uninstall_MPCImageSource_32.cmd ^
.\distrib\Uninstall_MPCImageSource_64.cmd ^
...
TITLE Compiling MPC Image Source [FINISHED]
TIMEOUT /T 3
ENDLOCAL
EXIT
...
MSBuild.exe MpcImageSource.sln %MSBUILD_SWITCHES%^
...
IF %ERRORLEVEL% NEQ 0 (
  CALL :SubMsg "ERROR" "MpcImageSource.sln %BUILDCFG% %1 - Compilation failed!"
) ELSE (
  CALL :SubMsg "INFO" "MpcImageSource.sln %BUILDCFG% %1 compiled successfully"
)
EXIT /B
